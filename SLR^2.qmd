...
The following code block will extract text from PDF files and structure them into a R dataframe. 
...
Get **the number of papers by year** overlay with **High-citation papers**.
...
ggplot(meta_basics_year, aes(x = Year, y = count)) +
  ...
  geom_text_repel(data = meta_basics_cites, aes(x = Year, y = Cites/10, label = ref), vjust = -0.5, color = "red") +
