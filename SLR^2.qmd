...
This code block contains all the packages required in this document.
...
The following code block will create a virtual environment `myenv` and install the Python package `PyMuPDF` for extracting text from PDF files. The Python script `extract-text-from-pdf.py` is used to extract text from PDF files.

Notes:

-   Our knowledge is that Python has a better library for extracting text from PDF files. Therefore, we use Python to extract text from PDF files.

-   After first time running the code block locally, it will automatically detect the Python environment and install it if not found.
...
reticulate::source_python("py/extract-text-from-pdf.py")
```
...
df <- tibble(doc = str_sub(list.files(data_root, pattern = "*.pdf"), 1, -5L), text) |> 
...
This following step is to extract the extracted text files from the PDF files, and sort them into a structured data frame.
...
text_df <- read_csv(file = "data/collection/textdf.csv")
data <- metadata |> left_join(text_df, by = c("id" = "id"))
```

## 0. Basic exploration of the meta information

Read the data.

To explore the keywords, we need to extract the three columns "id", "Keywords.Plus", and "Year" from the data frame.
...
```{r}
mapping <- st_read("data/map.csv",
                   options=c("X_POSSIBLE_NAMES=lng",
                             "Y_POSSIBLE_NAMES=lat"), 
                   crs=4326)

# If you wish to plot the boundaries of countries on the canvas
ggplot() + 
  geom_sf(data = mapping, aes(fill = "black")) + 
...
mapping.3.1 Summarise the number of papers within different countries
...
```{r}
mint::topic_words_to_ai_prompt(5, topic_names, 5)
```
