...
```{r}
write.csv(transposed_results_df, here("processed-data", "scopus_api_results.csv"), row.names = FALSE)
```
...
Something else we might want to consider, is if our literature search also identified one or more key articles that we know should be in the SLR. So, lets take a target article. In our case, the author of this specific literature review stated that [@van2006urban] and [@wu2022rethinking] should be considered as a target references. So let's examine, if they are included in our `all_unique_references` dataframe.
